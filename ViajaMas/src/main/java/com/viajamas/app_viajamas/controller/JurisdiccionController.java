...
public class JurisdiccionController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<DtoEntity> obtenerJurisdiccionXId(@PathVariable Integer id){
        Jurisdiccion jurisdiccionObtenido = iJurisdiccionService.obtenerJurisdiccionxId(id)
                .orElseThrow(() -> new ResourceNotFoundException("La jurisdiccion con Id" +
                        + id + " no existe"));
        DtoEntity jurisdiccionObtenidoDto = new DtoUtil().convertirADto(jurisdiccionObtenido, new JurisdiccionDto());
        return new ResponseEntity<>(jurisdiccionObtenidoDto,
                HttpStatus.OK);
    }

    @PostMapping("")
    public ResponseEntity<DtoEntity> registrarJurisdiccion(@RequestBody JurisdiccionDto jurisdiccionDto) {
        ...
        DtoEntity nuevaJurisdiccionDto = new DtoUtil().convertirADtoAnidado(result, new JurisdiccionDto());
        return new ResponseEntity<>(nuevaJurisdiccionDto, HttpStatus.CREATED);
    }
